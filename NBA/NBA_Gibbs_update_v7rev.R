...
x<-matrix(0,1230,181)
x[,1]<- 1
month <- c(rep(1, 246), rep(2, 219), rep(3, 228), rep(4, 177), rep(5, 237), rep(6, 123))
for (i in 1:1230){
	ix.home <- n.raw1[i, 1] - 1
	ix.visit <- n.raw[i, 3] - 1
	x[i, 1 + 6*ix.home + month[i]] <- 1
	x[i, 1 + 6*ix.visit + month[i]] <- -1
}
# for(i in 1:246){
...
# home.win<- (n.raw1[, 2]<n.raw1[,4])     ########  home team loses!
...
#variance########################## 
 # rho<- 0.05
 # sigma0<-exp(-rho*((outer(1:6,1:6,"-"))^2))
rho<- 0.95
sigma0<-matrix(rho,6,6)^(abs(outer(1:6,1:6,"-")))
 Lambda1 <- bdiag(1, kronecker(diag(30), solve(sigma0)))
...
beta1 <- betanew
lp.rho <- -.5*t(betanew)%*%(Lambda1%*%betanew) - 15*log(det(sigma0))
mu<-x%*%betanew
#################
xtranspose<-t(x)
set.seed(234567)
...
for(n in 1:N){
		...
		# if(rhostar<0)r1<-0
		if(rhostar>=1 || rhostar<=-1)r1<-0
		else{
 # sigma0star<-exp(-rhostar*((outer(1:6,1:6,"-"))^2))
# Lambda1star <- bdiag(1, kronecker(diag(30),solve(sigma0star)))
#			lp.rhostar <- -.5*t(beta1)%*%(Lambda1star%*%beta1) - 15*log(det(sigma0star))-b*rhostar
			sigma0star<-matrix(rhostar,6,6)^(abs(outer(1:6,1:6,"-")))
			Lambda1star <- bdiag(1, kronecker(diag(30),solve(sigma0star)))
			lp.rhostar <- -.5*t(beta1)%*%(Lambda1star%*%beta1) - 15*log(det(sigma0star))
			r1<-as.numeric(exp(lp.rhostar - lp.rho))
			if(runif(1) < r1){
				rho<-rhostar
				sigma0 <- sigma0star
				Lambda1 <- Lambda1star
...
v<-z%*%xtranspose
	    ...
	    beta1<-t(rmvnorm(1,mubeta,sigmabeta))
		output[n,]<-beta1
		mu<-x%*%beta1
#		lp.rho <- -.5*t(beta1)%*%(Lambda1%*%beta1) - 15*log(det(sigma0))-b*rho
		lp.rho <- -.5*t(beta1)%*%(Lambda1%*%beta1) - 15*log(det(sigma0))
